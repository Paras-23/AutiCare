...
class PieChartUIView: UIView {
    ...
    init(frame: CGRect, data: [CGFloat], colors: [UIColor], titles: [String], innerRadiusRatio: CGFloat = 0.5, separationAngle: CGFloat = 2 * .pi / 180) {
        self.data = data
        self.colors = colors
        self.titles = titles
    ...
    override func draw(_ rect: CGRect) {
        ...
        for (index, value) in data.enumerated() {
            let endAngle = startAngle + 2 * .pi * (value / total) - separationAngle
